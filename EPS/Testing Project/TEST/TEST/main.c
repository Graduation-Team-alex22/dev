...
void USART2_Config(void)
	...
	*/
	...
	hdma_usart2_rx.DMA_PeripheralBaseAddr = (uint32_t)&(USART3->DR);
	...
	DMA_Init(DMA1_Stream1, &hdma_usart2_rx);
...
usart_rx_check(void) {
    ...
    pos = ARRAY_LEN(usart_rx_dma_buffer) - DMA_GetCurrDataCounter(DMA1_Stream1);
