...
void MONITTORi_Stop(void)
   ...
   // Execution time in microseconds from DWT counter (runs at 22.5 MHz)
   uint32_t Execution_time_us = DWT->CYCCNT / 22.5;
   ...
   if ((Execution_time_us + Task_t_variation_g) < Task_BCET_g)
      ...
      if (PROCESSOR_Get_MoSt() == STARTUP_06b_MONITTOR_URUN_CHECK_M)
         ...
         {
         // This is *not* expected behaviour: treated as Fatal Platform Failure
      ...
      if (PROCESSOR_Get_MoSt() == STARTUP_06b_MONITTOR_URUN_CHECK_M)
         {
         // Should have detected underrun: Treated as Fatal Platform Failure
