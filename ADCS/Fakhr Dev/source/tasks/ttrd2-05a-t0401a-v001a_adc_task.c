...
static uint32_t ADC1_Read_Channel(uint32_t channel, uint32_t ADC_SampleTime); 
...
void ADC1_Init(void)
  ...
  if ((ADC_ref < ADC1_REF_MIN) || (ADC_ref > ADC1_REF_MAX))
   ...
   ADC1_temperature_raw_g = ADC1_Read_Channel(ADC_Channel_18, ADC_SampleTime_480Cycles);  
...
uint32_t ADC1_Update(void)
   ...
   uint32_t ADC_temp = ADC1_Read_Channel(ADC_Channel_18, ADC_SampleTime_480Cycles);      
