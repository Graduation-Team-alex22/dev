...
/**
  ...
  */
SD_Error SD_ReadBlock_Polling (uint32_t *pReadBuffer, uint64_t ReadAddr, uint32_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  /* Send stop transmission command in case of multiblock read */	
	if (SDIO_GetFlagStatus(SDIO_FLAG_DATAEND) && (NumberOfBlocks > 1U))
	{
...
/**
  ...
  * @retval SD Card error state
  */
DRESULT SD_SDIO_ReadBlocks(uint32_t *pData, uint64_t ReadAddr, uint32_t BlockSize, uint32_t NumOfBlocks)
{
	if( SD_ReadBlock_Polling (pData, ReadAddr, BlockSize, NumOfBlocks) != SD_OK )
	{
		return RES_ERROR;
	}
	else
...
/**
  ...
  * @retval SD Card error state
  */
SD_Error SD_WriteBlock_Polling (uint32_t *pWriteBuffer, uint64_t WriteAddr, uint32_t BlockSize, uint32_t NumberOfBlocks)
{
	SD_Error errorstatus = SD_OK;
	uint32_t totalnumberofbytes = 0U, bytestransferred = 0, count = 0, restwords = 0;
...
/**
  ...
  * @retval SD status
  */
DRESULT SD_SDIO_WriteBlocks(uint32_t *pData, uint64_t WriteAddr, uint32_t BlockSize, uint32_t NumOfBlocks)
{
	if( SD_WriteBlock_Polling (pData, WriteAddr, BlockSize, NumOfBlocks) != SD_OK )
	{
		return RES_ERROR;
	}
	else
...
//	SDIO_Init (&SDIO_InitStructure);
  ...
  SDIO_Init (&SDIO_InitStructure);  //added
...
#if FATFS_SDIO_4BIT == 1
	TM_GPIO_InitAlternate(GPIOC, GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_12, TM_GPIO_OType_PP, TM_GPIO_PuPd_NOPULL, TM_GPIO_Speed_Fast, GPIO_AF_SDIO);
#else
	TM_GPIO_InitAlternate(GPIOC, GPIO_PIN_8 | GPIO_PIN_12, TM_GPIO_OType_PP, TM_GPIO_PuPd_UP, TM_GPIO_Speed_Fast, GPIO_AF_SDIO);
#endif

	TM_GPIO_InitAlternate(GPIOD, GPIO_PIN_2, TM_GPIO_OType_PP, TM_GPIO_PuPd_NOPULL, TM_GPIO_Speed_Fast, GPIO_AF_SDIO);
