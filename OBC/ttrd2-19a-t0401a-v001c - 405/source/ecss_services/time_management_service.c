...
void set_time_UTC(struct time_utc utc){
    ...
    OBC_sys_setTime(utc.hour, utc.min, utc.sec);
    OBC_sys_setDate(utc.weekday, utc.month, utc.day, utc.year);
}
...
void get_time_QB50(uint32_t *qb){
    ...
    struct time_utc utc;
    OBC_sys_getTime(&utc.hour, &utc.min, &utc.sec);
    OBC_sys_getDate(&utc.weekday, &utc.month, &utc.day, &utc.year);
    cnv_UTC_QB50(utc, qb);
}
