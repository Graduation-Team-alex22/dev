...
SAT_returnState scheduling_service_load_schedules(void);
...
SAT_returnState scheduling_service_init(){
    ...
    scheduling_service_load_schedules();
		UART2_BUF_O_Write_String_To_Buffer("SCH PACKET LOADED\n");
...
static uint8_t pkt_num =0;    
    if( pkt_num == SC_MAX_STORED_SCHEDULES) 
		{
			pkt_num=0;
		}
       if (sch_mem_pool.sc_mem_array[pkt_num].pos_taken == true && /*if a valid schedule exists*/
           sch_mem_pool.sc_mem_array[pkt_num].enabled &&
           sc_s_state.schs_apids_enabled[(sch_mem_pool.sc_mem_array[pkt_num].app_id) - 1] == true){ /*if scheduling enabled for this APID */

           switch(sch_mem_pool.sc_mem_array[pkt_num].sch_evt){
                /*case ABSOLUTE:
               ...
               case REPETITIVE: ;                   
                   ...
                   if((!C_ASSERT(qb_time >= MIN_VALID_QB50_SECS)) == true ) { 
                    ...
                    if(sch_mem_pool.sc_mem_array[pkt_num].release_time <= qb_time){ /*time to execute*/
    	                UART2_BUF_O_Write_String_To_Buffer("SCHS ROUTING PKT\n");
    	                UART2_BUF_O_Send_All_Data();											
                        route_pkt(&(sch_mem_pool.sc_mem_array[pkt_num].tc_pck));
                        if(!(sch_mem_pool.sc_mem_array[pkt_num].timeout <=0)){ /*to save the else, and go for rescheduling*/
                            sch_mem_pool.sc_mem_array[pkt_num].release_time =
                                (sch_mem_pool.sc_mem_array[pkt_num].release_time + sch_mem_pool.sc_mem_array[pkt_num].timeout);
                            sch_mem_pool.sc_mem_array[pkt_num].pos_taken = true;
                            sch_mem_pool.sc_mem_array[pkt_num].enabled = true;
													pkt_num++;
    	                UART2_BUF_O_Write_String_To_Buffer("SCHS SCHEDULE RESCHEDULED\n");
    	                UART2_BUF_O_Send_All_Data();													
                        }/*timeout field is positive */
                        sch_mem_pool.sc_mem_array[pkt_num].pos_taken = false;
                        sch_mem_pool.sc_mem_array[pkt_num].enabled = false;
                        sc_s_state.nmbr_of_ld_sched--;
                        sc_s_state.sch_arr_full = false;
												pkt_num++;
    	                UART2_BUF_O_Write_String_To_Buffer("SCHS SCHEDULE POS FREED\n");
    	                UART2_BUF_O_Send_All_Data();												
                    }
             }
        }
...
SAT_returnState scheduling_service_load_schedules(){
    ...
    for(uint8_t s=0;s<SC_MAX_STORED_SCHEDULES;s++){
        ...
        if( state == SATR_OK){
						UART2_BUF_O_Write_String_To_Buffer("LOADING SCH PACKET... \r\n");
	          UART2_BUF_O_Send_All_Data();
            uint16_t f_s=0;
