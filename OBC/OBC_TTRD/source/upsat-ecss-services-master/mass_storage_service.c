...
#include "obc.h"
#include "../tasks/Mass_Storage_task.h" 
...
SAT_returnState mass_storage_app(tc_tm_pkt *pkt) {
    ...
    if(pkt->ser_subtype == TC_MS_DISABLE) {
        ...
        if(res == SATR_OK) { 
            ...
            if(mass_storage_dirCheck() != SATR_OK) 
             {
							 res = SATRF_DIR_ERROR;
						 }
						 else
...
static uint8_t loop1 =0;
static uint32_t cond1 =0;
static uint8_t loop2 =0;
static uint32_t cond2 =0;
static uint8_t loop3 =0;
static uint32_t cond3 =0;
static uint8_t loop4 =0;
static uint8_t loop5 =0;
static uint8_t loop6 =0;
SAT_returnState mass_storage_FORMAT() {
    ...
    *obc_data.fs_ev_tail = 1;
    ...
    for(; loop1 < 5 ;) {
        ...
        //HAL_sys_delay(5000);
				else if( cond1 >=13000 && cond1 <15000 )
				{
          power_control_api(OBC_SD_DEV_ID, P_ON);
					break;					
				}				
    ...
    //HAL_Delay(100);
    else if (cond1 ==0 && cond2 ==100)
		{
			if( cond3 == 0)
			{
				for(; loop2 < 5; )
			   {	 
				   if((res = f_mkdir(MS_WOD_LOG)) == FR_OK || res == FR_EXIST)
            {
					    loop2 =5;
							cond3 =1;
					    break;
			...
			else if( cond3 == 1 )
			{
				for(; loop3 < 5; )
				{
          if((res = f_mkdir(MS_EXT_WOD_LOG)) == FR_OK || res == FR_EXIST)
           {
						 loop3 =5;
             cond3 =2;						 
					   break; 
			...
			else if( cond3 == 2 )
			{
				for(; loop4 < 5; )
				{
          if((res = f_mkdir(MS_EVENT_LOG)) == FR_OK || res == FR_EXIST)
           {
						 loop4 =5;
             cond3 =3;						 
					   break; 
			...
			else if( cond3 == 3 )
			{
				for(; loop5 < 5; )
				{
          if((res = f_mkdir(MS_FOTOS)) == FR_OK || res == FR_EXIST)
           {
						 loop5 =5;
             cond3 =4;						 
					   break; 
			...
			else if( cond3 == 4 )
			{
				for(; loop6 < 5; )
				{
          if((res = f_mkdir(MS_SCHS)) == FR_OK || res == FR_EXIST)
           {
						 loop6 =5;
             cond3 =5;						 
					   break; 
