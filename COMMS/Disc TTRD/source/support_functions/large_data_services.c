...
SAT_returnState large_data_firstRx_api(tc_tm_pkt *pkt)
{
	...
	UART2_BUF_O_Write_String_To_Buffer("LD RX: First Frame");
	lid = pkt->data[0]; 
	 ...
	 * destination....
	 */
	if(LD_status.state == LD_STATE_RECEIVING && LD_status.ld_num == ld_num && LD_status.rx_lid == lid) 
	{
		/*Re-send the ACK */
...
SAT_returnState large_data_intRx_api(tc_tm_pkt *pkt)
{
	...
	UART2_BUF_O_Write_String_To_Buffer("LD RX: Frame ");
	UART2_BUF_O_Write_Number04_To_Buffer(ld_num);
	UART2_BUF_O_Write_String_To_Buffer(", Len ");
	...
	memcpy(&LD_status.buf[LD_status.rx_size], &pkt->data[LD_PKT_DATA_HDR_SIZE], size);
...
SAT_returnState large_data_lastRx_api(tc_tm_pkt *pkt)
{
	...
	UART2_BUF_O_Write_String_To_Buffer("LD RX: Last frame");
	if(!C_ASSERT(pkt != NULL && pkt->data != NULL))
	{
		return SATR_ERROR;
	}

	if(!C_ASSERT(LD_status.state == LD_STATE_RECEIVING || LD_status.state == LD_STATE_RECV_OK)) 
	{
		return SATR_ERROR;
	}
...
SAT_returnState large_data_downlinkTx_api(tc_tm_pkt *pkt)
{
	...
	for(i = 0; i < LD_status.tx_pkt; i++) 
	{
			...
			if(size > LD_PKT_DATA)
				size = LD_PKT_DATA; //take the minimum

			/*
			 * Copy the proper amount of data into the sub-frames and send them!
			 */
			memcpy(temp_pkt->data + LD_PKT_DATA_HDR_SIZE, LD_status.buf + (i * LD_PKT_DATA), size);

			//SYSVIEW_PRINT("LD TX: Frame %u", i);
			UART2_BUF_O_Write_String_To_Buffer("LD TX: Frame ");
			UART2_BUF_O_Write_Number04_To_Buffer(i);
			if (i == 0) 
			{
			...
			else if (i == LD_status.tx_pkt - 1) 
			{
				...
				UART2_BUF_O_Write_String_To_Buffer("LD TX: Last frame");
			 ...
			 */
			//HAL_Delay(200);// not needed in our system
	}
...
SAT_returnState large_data_retryTx_api(tc_tm_pkt *pkt)
{
	...
	UART2_BUF_O_Write_String_To_Buffer("LD TX: Retrying ");
	UART2_BUF_O_Write_Number04_To_Buffer(ld_num);
	...
	memcpy(temp_pkt->data + LD_PKT_DATA_HDR_SIZE, LD_status.buf + (ld_num * LD_PKT_DATA), size);
...
{
  uint32_t tmp_time = getCurrentTick ();
  ...
  if(LD_status.timeout != 0) 
    ...
    if (((tmp_time - LD_status.timeout) > timeout_period) || ((tmp_time - LD_status.started) > LD_MAX_TRANSFER_TIME) ) 
		{
      large_data_timeout ();
      LD_status.state = LD_STATE_FREE;
      LD_status.ld_num = 0;
...
SAT_returnState large_data_timeout(void)
{
	...
	UART2_BUF_O_Write_String_To_Buffer("LD RX: Timeout");
