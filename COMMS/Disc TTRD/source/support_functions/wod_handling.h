...
//Getters
int32_t
comms_wod_send(uint8_t bypass_check); // originally comms_wod_tx

int32_t
comms_ex_wod_send(); // originally comms_ex_wod_tx
...
uint8_t
bus_5000mV_current_valid(uint8_t val); // this function returns a call to the getter bus_3300mV_current_valid

uint8_t
comms_temp_valid(uint8_t val);

uint8_t
eps_temp_valid(uint8_t val); // this function returns a call to the getter comms_temp_valid

uint8_t
bat_temp_valid(uint8_t val); // this function returns a call to the getter comms_temp_valid

//Setters
void
store_wod_obc(const uint8_t *obc_wod, size_t len); // obc_wod is the buffer with the WOD, as it comes from the OBC
